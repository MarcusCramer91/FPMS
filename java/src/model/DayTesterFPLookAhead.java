...
public class DayTesterFPLookAhead {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int lookAheadStage : lookAheadStages) {
			...
			try {
				DayTesterFPLookAhead tester = new DayTesterFPLookAhead(distmat, orders);
	...
	public DayTesterFPLookAhead(DistanceMatrix distmat, ArrayList<Order> orders) {
	...
	public void getColgenCosts(int startingTime, int endTime, int lookAheadStage) throws Exception {
		...
		for (int time = startingTime; time <= endTime; time += 60) {
			...
			if (orders.size() > 0 && (numberOfAvailableOrders >= 40 || (time == endTime && orders.size() > 0) || 
