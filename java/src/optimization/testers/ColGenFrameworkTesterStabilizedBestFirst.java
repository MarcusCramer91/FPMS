...
import model.Order;
import optimization.CPlexTSP;
import optimization.CPlexTSPTW;
...
public class ColGenFrameworkTesterStabilizedBestFirst {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 20; i <= 50; i += 10) {
			for (int j = 1; j <= 10; j++) {
				...
				ColGenFrameworkTesterStabilizedBestFirst tester = new ColGenFrameworkTesterStabilizedBestFirst(distmat, orders, currentTime, id);
	...
	public ColGenFrameworkTesterStabilizedBestFirst(DistanceMatrix distmat, ArrayList<Order> orders, 
	...
	public void getRoutes(DistanceMatrix distmat, int compTimeLimit, int branchTimeLimit, 
		...
		// initialize mus with the Flaschenpost solution costs divided by the number of customers
		 ...
		 ArrayList<SearchTreeInstance> searchTreeInstances = new ArrayList<SearchTreeInstance>();
		 SearchTreeInstance sti = new SearchTreeInstance(distmat, compTimeLimit, branchTimeLimit, mus, 0, false, 
				 paths.size(), null, 0, 0);
		 searchTreeInstances.add(sti);
		 
		 while (System.currentTimeMillis() - startingTime < compTimeLimit * 1000 && searchTreeInstances.size() > 0) {
			 // find best instance
			 SearchTreeInstance best = null;
			 double bestRelaxedCosts = Double.MAX_VALUE;
			 for (SearchTreeInstance temp : searchTreeInstances) {
				 if (temp.getRelaxedCosts() < bestRelaxedCosts) {
					 bestRelaxedCosts = temp.getRelaxedCosts();
					 best = temp;
				 }
			 }
			 searchTreeInstances.remove(best);
			 SearchTreeInstance[] stis = getRoutesInternal(best.getDistmat(), best.getCompTimeLimit(), 
					 best.getBranchTimeLimit(), best.getMus(), best.getTreeDepth(), best.isVehicleBranchingAllowed(), 
					 best.getnVehicles(), best.getArcsBranchedOn());
			 if (stis != null) {
				 searchTreeInstances.add(stis[0]);
				 searchTreeInstances.add(stis[1]);
			 }
		 }
		 ...
		 FileWriter writer = new FileWriter("C:\\Users\\Marcus\\Documents\\FPMS\\results\\colgen\\stabilizedBestFirst\\_Summary" + id + ".csv", true);
	...
	private SearchTreeInstance[] getRoutesInternal(DistanceMatrix distmat, int compTimeLimit,
		...
		if (System.currentTimeMillis() - startingTime > compTimeLimit * 1000) return null;
		...
		ArrayList<double[]> musPerIteration = new ArrayList<double[]>();
	     ...
	     while (System.currentTimeMillis() - branchingTime < branchTimeLimit * 1000 &&
	    	...
	    	// logging
			 ...
			 if (mipResult != null) {
				 ...
				 FileWriter writer = new FileWriter("C:\\Users\\Marcus\\Documents\\FPMS\\results\\colgen\\stabilizedBestFirst\\" + id + ".csv", true);
		 ...
		 SearchTreeInstance[] res = new SearchTreeInstance[2];
	     ...
	     SearchTreeInstance sti1 = new SearchTreeInstance(distmat1, compTimeLimit, branchTimeLimit, mus, treeDepth + 1,
	    		 vehicleBranchingAllowed, nVehicles, newArcsBranchedOn, relaxedCosts, costs);
	     ...
	     SearchTreeInstance sti0 = new SearchTreeInstance(distmat0, compTimeLimit, branchTimeLimit, mus, treeDepth + 1,
	    		 vehicleBranchingAllowed, nVehicles, newArcsBranchedOn, relaxedCosts, costs);
	     res[0] = sti1;
	     res[1] = sti0;
	     return res;
	...
	private class SearchTreeInstance {
		...
		double[] mus;
		int compTimeLimit;
		boolean vehicleBranchingAllowed;
		int nVehicles;
		ArrayList<Integer[]> arcsBranchedOn;
		double relaxedCosts;
		double mipCosts;
		...
		SearchTreeInstance(DistanceMatrix distmat, int compTimeLimit,
				int branchTimeLimit, double[] mus, int treeDepth, 
				boolean vehicleBranchingAllowed, int nVehicles, ArrayList<Integer[]> arcsBranchedOn,
				double relaxedCosts, double mipCosts) {
			this.distmat = distmat;
			this.branchTimeLimit = branchTimeLimit;
			this.treeDepth = treeDepth;
			this.mus = mus;
			this.vehicleBranchingAllowed = vehicleBranchingAllowed;
			this.arcsBranchedOn = arcsBranchedOn;
			this.relaxedCosts = relaxedCosts;
			this.mipCosts = mipCosts;
		...
		public double getRelaxedCosts() {
			return relaxedCosts;
		}

		public void setRelaxedCosts(double relaxedCosts) {
			this.relaxedCosts = relaxedCosts;
		}

		public double getMipCosts() {
			return mipCosts;
		}

		public void setMipCosts(double mipCosts) {
			this.mipCosts = mipCosts;
		}
		...
		public int getCompTimeLimit() {
			return compTimeLimit;
		}



		public void setCompTimeLimit(int compTimeLimit) {
			this.compTimeLimit = compTimeLimit;
		}



		public boolean isVehicleBranchingAllowed() {
			return vehicleBranchingAllowed;
		}



		public void setVehicleBranchingAllowed(boolean vehicleBranchingAllowed) {
			this.vehicleBranchingAllowed = vehicleBranchingAllowed;
		}
		...
		public ArrayList<Integer[]> getArcsBranchedOn() {
			return arcsBranchedOn;
		}



		public void setArcsBranchedOn(ArrayList<Integer[]> arcsBranchedOn) {
			this.arcsBranchedOn = arcsBranchedOn;
		}
