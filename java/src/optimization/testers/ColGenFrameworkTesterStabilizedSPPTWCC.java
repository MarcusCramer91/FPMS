...
import optimization.pricingproblem.SPPTWCC;
import util.DistanceMatrixImporter;
import util.OrdersImporter;

public class ColGenFrameworkTesterStabilizedSPPTWCC {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 40; i <= 50; i += 10) {
			for (int j = 1; j <= 10; j++) {
				...
				ColGenFrameworkTesterStabilizedSPPTWCC tester = new ColGenFrameworkTesterStabilizedSPPTWCC(distmat, orders, currentTime, id);
	...
	public ColGenFrameworkTesterStabilizedSPPTWCC(DistanceMatrix distmat, ArrayList<Order> orders, 
	...
	public void getRoutes(DistanceMatrix distmat, int compTimeLimit, int branchTimeLimit, 
		...
		// initialize mus with the Flaschenpost solution costs divided by the number of customers
		 ...
		 getRoutesInternal(distmat, compTimeLimit, branchTimeLimit, mus, 0, true, false, paths.size());
	}
		...
		ArrayList<double[]> musPerIteration = new ArrayList<double[]>();
	     ...
	     while (System.currentTimeMillis() - branchingTime < branchTimeLimit * 1000 &&
			 ...
			 if (mipResult != null) {
				 ...
				 FileWriter writer = new FileWriter("C:\\Users\\Marcus\\Documents\\FPMS\\results\\colgen\\stabilizedSPPTWCC\\" + id + ".csv", true);
