...
public class DayTesterPPProblemSize {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < problemSizes.length; i++) {
			...
			int endTime = 43200; // 9 pm
				...
				DayTesterPPProblemSize tester = new DayTesterPPProblemSize(distmat, orders);
				tester.getFlaschenPostCosts(startingTime, endTime, problemSizes[i]);
		}
	}
	
	public DayTesterPPProblemSize(DistanceMatrix distmat, ArrayList<Order> orders) {
		this.distanceMatrix = distmat;
		this.orders = orders;
	}
	
	public void getFlaschenPostCosts(int startingTime, int endTime, int problemSize) throws Exception {
		for (int time = startingTime; time <= endTime; time += 60) {
			if (orders.size() > 0 && (numberOfAvailableOrders(time) >= problemSize || (time == endTime && orders.size() > 0) || 
				...
				handleMETsAndCosts(time, routes, problemSize);
			}
		}
	}
	...
	private void log(int time, ArrayList<Order[]> routes, double drivingTime, double overallTime, 
		...
		String rootPath = new File("").getAbsolutePath();
		rootPath = rootPath.substring(0, rootPath.length() - 5);
		FileWriter writer = new FileWriter(rootPath  + "/results/days/" + "_" + problemSize + "cust_FP_Day1.csv", true);
