...
public class DayTesterPPMETGoodness {
	...
	public static void main(String[] args) throws Exception {
		...
		//int[] metThresholds = {72*60, 74*60, 76*60, 78*60, 80*60, 82*60};
		int[] metThresholds = {66*60};
		for (int metThreshold : metThresholds) {
			...
			try {
				DayTesterPPMETGoodness tester = new DayTesterPPMETGoodness(distmat, orders);
	...
	public DayTesterPPMETGoodness(DistanceMatrix distmat, ArrayList<Order> orders) {
	...
	private void log(int time, ArrayList<Order[]> routes, double drivingTime, double overallTime, 
		...
		String rootPath = new File("").getAbsolutePath();
		rootPath = rootPath.substring(0, rootPath.length() - 5);
		FileWriter writer = new FileWriter(rootPath + "/results/days/" + metThreshold + "_fp_routeGoodnessMET_Day1.csv", true);
