...
public class DayTesterPPLookAhead {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int lookAheadStage : lookAheadStages) {
			...
			try {
				DayTesterPPLookAhead tester = new DayTesterPPLookAhead(distmat, orders);
	...
	public DayTesterPPLookAhead(DistanceMatrix distmat, ArrayList<Order> orders) {
	...
	public void getColgenCosts(int startingTime, int endTime, int lookAheadStage) throws Exception {
		...
		for (int time = startingTime; time <= endTime; time += 60) {
			...
			if (PPOptimize.checkOptimizationNecessity(time, orders, 30) || time == endTime || 
	...
	private void log(int time, ArrayList<Order[]> routes, double drivingTime, double overallTime, 
		...
		String rootPath = new File("").getAbsolutePath();
		rootPath = rootPath.substring(0, rootPath.length() - 5);
		FileWriter writer = new FileWriter(rootPath + "/results/days/" + lookAheadStage + "_fp_lookAhead_Day1.csv", true);
