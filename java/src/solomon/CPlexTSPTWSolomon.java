...
public class CPlexTSPTWSolomon {   
   ...
   public static int[] getRoute(DistanceMatrix distmat, ArrayList<Order> orders, int currentTime) {
	     ...
	     int[] timesReady = new int[nLocations];
	     timesRemaining[0] = 0;
	     timesReady[0] = 0;
	     for (int i = 1; i < nCustomers+1; i++) {
	    	 timesRemaining[i] = orders.get(i-1).getLatest();
	    	 timesReady[i] = orders.get(i-1).getEarliest();
	     }
	     timesRemaining[timesRemaining.length-1] = 2000;
	     timesReady[timesReady.length-1] = 0;
		 double maxValue = 0;
